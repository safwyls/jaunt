...
namespace Jaunt.Hud
{
    public class HudIconRenderer : IRenderer, IDisposable
    {
        ...
        protected void OnGameTick(float dt)
        {
            ...
            if (player.MountedOn?.MountSupplier?.OnEntity?.GetBehavior<EntityBehaviorGait>()?.CurrentGait is JauntGaitMeta jauntGait && jauntGait.IconTexture is not null)
            {
                activeTexture = texturesDict.TryGetValue(jauntGait.IconTexture, out LoadedTexture value) ? value : texturesDict["empty"];
            }
            else
            {
